...
PositionLogger::PositionLogger(SkydelNotifierInterface *skydelNotifier,
    ...
    : m_skydelNotifier(skydelNotifier), config(config),
      m_simulationFile(config->simulationLogPath + QDir::separator() +
                       "simulation_position_observer_output.csv"),
      m_receiverFile(config->receiverLogPath + QDir::separator() +
  ...
  if (config->simulationFileLogging) {
    if (!m_simulationFile.open(QIODevice::WriteOnly | QIODevice::Append |
                               QIODevice::Text)) {
  ...
  if (config->receiverFileLogging) {
    if (!m_receiverFile.open(QIODevice::WriteOnly | QIODevice::Append |
                             QIODevice::Text)) {
...
void PositionLogger::pushPosition(const TimedPosition &p) {
  ...
  QString formatedSimulationPosition =
      QString::number(p.time) + ",Simulation," + toString(sPosition);
  QString formatedReceiverPosition =
      QString::number(p.time) + ",Receiver," + toString(rPosition);

  if (config->simulationNetworkLogging) {
    QByteArray byteArray;
    byteArray.append(formatedSimulationPosition.toUtf8());
    if (m_simulationSocket.writeDatagram(
            byteArray, config->simulationNetworkLogAddress,
            config->simulationNetworkLogPort) == -1) {
      throw std::runtime_error(
          m_simulationSocket.errorString().toStdString().c_str());
    }
  }
  ...
  if (config->simulationFileLogging) {
    if (!m_simulationFile.isOpen()) {
      if (!m_simulationFile.open(QIODevice::WriteOnly | QIODevice::Append |
                                 QIODevice::Text)) {
        throw std::runtime_error(
            "PositionLogger - Can't open simulation file.");
      }
    }
    ...
    if (stream.status() == QTextStream::WriteFailed) {
      throw std::runtime_error(
          "PositionLogger - Failed to write in simulation file.");
    }
  }
  ...
  if (config->receiverNetworkLogging) {
    ...
    if (m_receiverSocket.writeDatagram(byteArray,
                                       config->receiverNetworkLogAddress,
                                       config->receiverNetworkLogPort) == -1) {
      throw std::runtime_error(
          m_receiverSocket.errorString().toStdString().c_str());
    }
  }
  ...
  if (config->receiverFileLogging) {
    ...
    if (stream.status() == QTextStream::WriteFailed) {
      throw std::runtime_error(
          "PositionLogger - Failed to write in receiver file.");
    }
  }
}
