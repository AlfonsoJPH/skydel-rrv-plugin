...
#include <iostream>
/**
 * # NMEA message parser library
 */
/**
 * @brief Enum for each different NMEA messages types
 */
enum MessagesTypes {
 ...
 */
/**
 * @brief Function to check if the message has the correct checksum
 *
 * @param message The message to check
 * @param checksum The checksum to compare
 * @return true if the message has the correct checksum
 * @return false if the message has the incorrect checksum
 */
static bool checkMessageIntegrity(const QString &message,
                                  const QString &checksum) {
  int checksumValue = 0;
  for (int i = 1; i < message.size() && message[i] != '*'; ++i) {
    checksumValue ^= message[i].unicode();
  }
  QString checksumCalculated =
      QString("%1").arg(checksumValue, 2, 16, QLatin1Char('0')).toUpper();
  return checksumCalculated == checksum;
}

/**
 * @brief Function to parse the UTC time from a string
 *
 * @param time The time string to parse
 * @return UTC_Time The UTC time struct
 */
...
/**
 * @brief Function to parse the message type from a message
 *
 * @param message The message to parse
 * @return MessagesTypes The message type
 */
static MessagesTypes getMessageType(const QStringList &message) {
  try {
    QString ID = message[0];
    if (ID.endsWith("GGA"))
      return MessagesTypes::GGA;
    if (ID.endsWith("GLL"))
      return MessagesTypes::GLL;
    if (ID.endsWith("GSA"))
      return MessagesTypes::GSA;
    if (ID.endsWith("GSV"))
      return MessagesTypes::GSV;
    if (ID.endsWith("MSS"))
      return MessagesTypes::MSS;
    if (ID.endsWith("RMC"))
      return MessagesTypes::RMC;
    if (ID.endsWith("VTG"))
      return MessagesTypes::VTG;
    if (ID.endsWith("ZDA"))
      return MessagesTypes::ZDA;
...
/**
 * @brief Function to parse a GGA message
 *
 * @param message The message to parse
 * @param data The GGA struct to fill
...
static bool GGAparser(const QStringList &message, struct GGA &data) {
  ...
  try {
    if (message.size() >= 15) {
      ...
      gga.altitudeMSL =
          message[9].isEmpty() ? 0.0 : message[9].toDouble(); // can be empty
      gga.altitudeUnits =
          message[10].isEmpty() ? ' ' : message[10][0]; // can be empty
      gga.geoidSeparation =
          message[11].isEmpty() ? 0.0 : message[11].toDouble(); // can be empty
      gga.geoidSeparationUnits =
          message[12].isEmpty() ? ' ' : message[12][0]; // can be empty
      gga.ageDiffCorr =
          message[13].isEmpty() ? 0.0 : message[13].toDouble(); // can be empty
      gga.diffRefStationID =
          message[14].isEmpty() ? "Unknown" : message[14]; // can be empty

      // Now the checksum would be checked, but it is not working properly and
      // may be inneficient
    }
  } catch (const std::exception &e) {
    std::cerr << e.what() << '\n';
...
/**
 * @brief Function to parse a GLL message
 *
 * @param message The message to parse
 * @param data The GLL struct to fill
 * @return true if the message is valid
 */
...
/**
 * @brief Function to parse a GSA message
 *
 * @param message The message to parse
 * @param data The GSA struct to fill
 * @return true if the message is valid
 */
